...
<body bgcolor="yellow"><br>
  ...
  <section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        // Insert section
        $gms = $connection->prepare("INSERT INTO submission(Id, user, assignment, exam, time, score) VALUES (?, ?, ?, ?, ?, ?)");
        $gms->bind_param("isssss", $id, $user, $assignment, $exam, $time, $score);
        ...
        if ($gms->execute()) {
            echo "New record has been added successfully.<br><br>
                 <a href='submission.html'>Back to Form</a>";
        } else {
    ...
    <table>
        ...
        <?php
        include('db.php');
        // SQL query to fetch data from the submissio table
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td>" . $row["time"] . "</td>
                    <td>" . $row["score"] . "</td>
                    <td><a style='padding:4px' href='delete_submission.php?Id=$id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_submission.php?Id=$id'>Update</a></td> 
                </tr>";
            }
        } else {
